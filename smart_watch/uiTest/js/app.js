 ...
 *              
 *              This software has been significantly modified.
 *
...
(function() {
    ...
    /**
     * Similar to createPageChangeFunc but links to the settings page instead of the contact page
     */
    function createPageChangeToSettings(data) {
    	return function() {
    		var elmName = document.querySelector("#name-settings"),
    			elmNum = document.querySelector("#number-settings");
    		
    		emptyElement(elmName.appendChild(document.createTextNode(PERSON_DATA_NAME[data])));
    		emptyElement(elmNum).appendChild(document.createTextNode(PERSON_DATA_NUM[data]));
    		
    		pageController.setPageBgImage("page-settings", PERSON_DATA_IMAGE[data]);
    		pageController.movePage("page-settings");
    	};
    }
    ...
    function pushData() {
        var i;
        if (PERSON_DATA_IMAGE[0]) {
        	listController.addData(PERSON_DATA_NAME[0], PERSON_DATA_IMAGE[0], createPageChangeToSettings(0));
        }
        for (i = 1; i < PERSON_DATA_NAME.length; i++) {
